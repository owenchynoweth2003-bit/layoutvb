...
>>>>>>> 1f1fec14fe1d31287d660e823928e29b1f4fc30d
      ...
      <Sc title="Cut Inspector" color="#fbbf24">
        {walls.map((w, i) => {
          ...
          const tmBad = hasTrueMin && trueMin < Math.min(mcw, mch);
          return (
            <div key={i} style={{ marginBottom: 8, padding: 8, borderRadius: 10, background: i === pro.mi ? '#f0f4ff' : '#fff', border: `1px solid ${i === pro.mi ? '#bfdbfe' : '#e5e7eb'}`, transition: 'all .2s' }}>
              ...
              {!hasC ? (
                <>
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 3 }}>
                    <CutBadge label="L" val={lo.lc} min={mcw} /><CutBadge label="R" val={lo.rc} min={mcw} />
                    <CutBadge label="T" val={lo.tc} min={mch} /><CutBadge label="B" val={lo.bc} min={mch} />
                  </div>
                  {hasTrueMin && (
