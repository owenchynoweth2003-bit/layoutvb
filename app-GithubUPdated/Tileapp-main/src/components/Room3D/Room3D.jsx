...
export default function Room3D({ walls, feats, wrap, activeWallIdx, tile, pat, grout, groutColor, pro, viewMode }) {
  const mountRef = useRef(null);
  const hudRef   = useRef(null);
  const { loaded, groupRef, texCacheRef, rendererRef } = useThreeScene(mountRef, hudRef);

  // Rebuild meshes when layout data changes
  useEffect(() => {
    if (!loaded || !groupRef.current) return;
    buildWalls(groupRef.current, walls, feats, wrap, activeWallIdx, tile, pat, grout, groutColor, pro, viewMode, texCacheRef, rendererRef);
  }, [loaded, walls, wrap, activeWallIdx, feats, tile, pat, grout, groutColor, pro, viewMode]);

  // Snap camera when viewMode changes
  useEffect(() => {
    if (!loaded || !groupRef.current) return;
